...
trait ReactiveClient {
  ...
  def request[In: ReactiveSerializer, Out: ReactiveDeserializer](destination: String, payload: In)(implicit timeout: Timeout, c: ClassTag[Out]): Future[Out]
...
class KafkaReactiveClient(settings: KafkaRClientSettings)(implicit system: ActorSystem) extends ReactiveClient {
  ...
  def request[In: ReactiveSerializer, Out: ReactiveDeserializer](destination: String, payload: In)(implicit timeout: Timeout, c: ClassTag[Out]): Future[Out] = {
    destination match {
      ...
      case _ => Future.failed(new Error("unknown destination"))
    }
  }
}
